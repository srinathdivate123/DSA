...
// In the next itertions, if elements we encounter are equal to the 'element' then we so count++ which means that the 'element' is the largest in the array till now
// If we get elements that are not equal to 'elenent' then we do count-- which means that the 'element' is not the majority so far.
...
void moores_Voting_Algorithm(int arr[], int n)
{
    ...
    // If it is stated that there will be a majority element in the array, then we don't need to de the below part. We can just print 'element' then!
