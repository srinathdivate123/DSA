// Given a chain of matrices A1,A2,...An denoted by an array of size n+1, find out the minimum number of operations to multiply these n matrices.
...
int Function(vector<int> &arr, int i, int j, vector<vector<int>> &dp)
{
    ...
    for (int k = i; k <= j - 1; k++)
    {
        int steps = (arr[i - 1] * arr[k] * arr[j]) + Function(arr, i, k, dp) + Function(arr, k + 1, j, dp);
        mini = min(mini, steps);
    }
