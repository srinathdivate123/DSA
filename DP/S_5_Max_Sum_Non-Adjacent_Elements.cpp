...
int memoization(int ind, vector<int> &arr, vector<int> &dp)
{
    ...
    int nonPick = 0 + memoization(ind - 1, arr, dp); // We're adding 0 as we do not pick
...
int tabulation(int n, vector<int> &arr, vector<int> &dp)
{
    ...
    {
        int pick = arr[i];
        if (i > 1) // This is so that we do not access negative indexes
            pick += dp[i - 2];
        int nonPick = 0 + dp[i - 1]; // We're adding 0 as we do not pick
