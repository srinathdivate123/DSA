...
// According to Wikipedia, every level, except possibly the last, is completely filled in a complete binary tree, and all nodes in the last level are as far left as possible. It can have between 1 and 2^h nodes inclusive at the last level h. Design an algorithm that runs in less than O(n) time complexity.
...
// The relationship between the height of the binary tree (h) and the maximum number of nodes it can have, is denoted by the formula:
// Maximum Number of Nodes: 2^h-1
// If the last level of a binary tree is perfectly filled, known as a perfect binary tree, then nodes=2^h-1. To check if the last level is filled, we can compare the left and right heights of the tree.
// If the left height equals right height, it indicates that the last level is completely filled.
// When left and right height differ, we recursively calculate the number of nodes in the left subtree and in the right subtree, and then return the total count as 1 + leftNodes + rightNodes.
...
public:
    int countNodes(TreeNode *root)
    {
        ...
        if (lh == rh)
        {
            // The number of nodes in the below subtre: 2^h - 1
            return (1 << lh) - 1;
        }
