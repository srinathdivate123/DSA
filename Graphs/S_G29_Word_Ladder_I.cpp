...
// The problem aims to transform a startWord into a targetWord by changing one character at a time, with each intermediate word present in a given wordList. The shortest sequence length is sought. We can model this problem as a graph:
...
// TC: O(N * L * 26), where N is the number of words in the list and L is the length of each word. For each word, we attempt to change each of its L characters to 26 possible letters.
